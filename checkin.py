...
def start():
    ...
    checkin = requests.post(url,headers={'cookie': cookie ,'referer': referer, "token": "glados.rocks" })
    state =  requests.get(url2,headers={'cookie': cookie ,'referer': referer , "token": "glados.rocks" })
    print(checkin.json())
